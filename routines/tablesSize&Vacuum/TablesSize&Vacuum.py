...
if dbutils.widgets.get('databaseTarget') == '':
  dbutils.notebook.exit("Variável databaseTarget não informada")

if dbutils.widgets.get('storageLocation') == '':
  dbutils.notebook.exit("Variável storageLocation não informada")
...
from
  ...
  left join (
    select
      identifier,
      tableName,
      Round(Sum(SizeKB) / 1024, 2) as storageSizeMB,
      Round(Sum(SizeKB) / 1024 / 1024, 2) as storageSizeGB,
