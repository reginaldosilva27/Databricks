...
# MAGIC | `tableSizesMonitor` | 'tableSizesMonitor' | Define nome da tabela para armazenar o resultado agregado da execução com detalhes no nível de tabela, caso não exista a tabela será criada |
...
# MAGIC > 1x Tabela para armazenar o resultado agregado da execução com detalhes no nivel de tabela, por padrão será chamada de **tableSizesMonitor**, essa tabela nunca é truncada <br>
...
# MAGIC ## Monitoramento
# MAGIC 
# MAGIC > Monitore seu ambiente através das tabelas tbVacuumSummary e tableSizesMonitor<br>
# MAGIC > A tabela **tbVacuumSummary** armazena 1 linha por execução de dados sumarizados<br>
# MAGIC > A tabela **tableSizesMonitor** armazena 1 linha por tabela por execução com dados sumarizados<br>
# MAGIC 
# MAGIC 
# MAGIC ## Benchmarch:
# MAGIC 
...
dbutils.widgets.text('tableSizesMonitor',     'tableSizesMonitor')
...
tableSizesMonitor     = dbutils.widgets.get('tableSizesMonitor')
...
tableSizesMonitorLocation = f'database={databaseCatalog}/table_name={tableSizesMonitor}' # Definir storage da tabela de resultado do dbutils
...
spark.sql(f"""
        CREATE TABLE IF NOT EXISTS {databaseCatalog}.{tableSizesMonitor} (
          identifier STRING,
          dateLoad date,
          databaseName STRING,
          tableName STRING,
        ...
        USING delta
        PARTITIONED BY (dateLoad)
        LOCATION '{storageLocation}{tableSizesMonitorLocation}'
        """)
...
spark.sql(f"""
INSERT INTO {databaseCatalog}.{tableSizesMonitor}
select
  t.identifier,
...
storageSizeGB = spark.sql(f""" select round(sum(storageSizeGB),2) as storageSizeGB from {databaseCatalog}.{tableSizesMonitor} WHERE identifier = {identifier}""").collect()[0][0]
actualTablesSizeGB = spark.sql(f""" select round(sum(actualTablesSizeGB),2) as actualtableSizeGB from {databaseCatalog}.{tableSizesMonitor} WHERE identifier = {identifier}""").collect()[0][0]
qtdTablesVacuum = spark.sql(f""" select count(*) from {databaseCatalog}.{tableSizesMonitor} WHERE identifier = {identifier} and StorageVsTable > {vacuumThreshold} """).collect()[0][0]
...
if runVacuum:
    ...
    tables = spark.sql(f"""select databaseName, tableName from {databaseCatalog}.{tableSizesMonitor} where StorageVsTable > {vacuumThreshold} and identifier = '{identifier}' """).collect()
...
spark.sql(f'select * from {databaseCatalog}.{tableSizesMonitor} WHERE identifier = {identifier}').display()
